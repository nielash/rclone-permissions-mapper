...
// lightly adapted from ncw's excellent original at: https://github.com/rclone/rclone/blob/master/bin/cross-compile.go
...
func buildZip(dir string) string {
	...
	run("cp", "-a", "../rclone-permissions-mapper.1", dir)
	if *gitLog != "" {
		run("cp", "-a", *gitLog, dir)
	}
	zip := dir + ".zip"
...
func compileArch(version, goos, goarch, dir string) bool {
	...
	output := filepath.Join(dir, "rclone-permissions-mapper")
	if goos == "windows" {
		output += ".exe"
	...
	args := []string{
		"go", "build",
		"--ldflags", "-s -X github.com/rclone-permissions-mapper/rclone-permissions-mapper/fs.Version=" + version,
		"-trimpath",
		"-o", output,
		"-tags", *tags,
	}
...
func compile(version string) {
	...
	for _, osarch := range osarches {
		...
		dir := filepath.Join("rclone-permissions-mapper-" + version + "-" + userGoos + "-" + goarch)
		run <- func() {
...
func main() {
	...
	chdir("build")
	err := os.WriteFile("version.txt", []byte(fmt.Sprintf("rclone-permissions-mapper %s\n", version)), 0666)
	if err != nil {
